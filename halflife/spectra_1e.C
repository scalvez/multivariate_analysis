...
void spectra_1e()
{
  ...
  TFile * f_0nu = TFile::Open("./spectra_1e/spectrum_0nu.root");
  TFile * f_2nu = TFile::Open("./spectra_1e/spectrum_2nu.root");
  TFile * f_tl208 = TFile::Open("./spectra_1e/spectrum_tl208.root");
  TFile * f_bi214 = TFile::Open("./spectra_1e/spectrum_bi214.root");
  TFile * f_radon = TFile::Open("./spectra_1e/spectrum_radon.root");
  ...
  TH1F *h_0nu = new TH1F("electron_spectrum_0nu","electron_spectrum_0nu",100,0,4);
  TH1F *h_2nu = new TH1F("electron_spectrum_2nu","electron_spectrum_2nu",100,0,4);
  TH1F *h_tl208 = new TH1F("electron_spectrum_tl208","electron_spectrum_tl208",100,0,4);
  TH1F *h_bi214 = new TH1F("electron_spectrum_bi214","electron_spectrum_bi214",100,0,4);
  TH1F *h_radon = new TH1F("electron_spectrum_radon","electron_spectrum_radon",100,0,4);

  double electron_energy_0nu = 0;
  tree_0nu->SetBranchAddress("1e_electron_energy",&electron_energy_0nu);
  int nentries_0nu = tree_0nu->GetEntriesFast();
  for(int i = 0; i< nentries_0nu; ++i) {
    tree_0nu->GetEntry(i);
    // std::cout << electron_energy_0nu << std::endl;
    h_0nu->Fill(electron_energy_0nu);
  }

  double electron_energy_2nu = 0;
  tree_2nu->SetBranchAddress("1e_electron_energy",&electron_energy_2nu);
  int nentries_2nu = tree_2nu->GetEntriesFast();
  for(int i = 0; i< nentries_2nu; ++i) {
    tree_2nu->GetEntry(i);
    // std::cout << electron_energy_2nu << std::endl;
    h_2nu->Fill(electron_energy_2nu);
  }

  double electron_energy_tl208 = 0;
  tree_tl208->SetBranchAddress("1e_electron_energy",&electron_energy_tl208);
  int nentries_tl208 = tree_tl208->GetEntriesFast();
  for(int i = 0; i< nentries_tl208; ++i) {
    tree_tl208->GetEntry(i);
    // std::cout << electron_energy_tl208 << std::endl;
    h_tl208->Fill(electron_energy_tl208);
  }

  double electron_energy_bi214 = 0;
  tree_bi214->SetBranchAddress("1e_electron_energy",&electron_energy_bi214);
  int nentries_bi214 = tree_bi214->GetEntriesFast();
  for(int i = 0; i< nentries_bi214; ++i) {
    tree_bi214->GetEntry(i);
    // std::cout << electron_energy_bi214 << std::endl;
    h_bi214->Fill(electron_energy_bi214);
  }

  double electron_energy_radon = 0;
  tree_radon->SetBranchAddress("1e_electron_energy",&electron_energy_radon);
  int nentries_radon = tree_radon->GetEntriesFast();
  for(int i = 0; i< nentries_radon; ++i) {
    tree_radon->GetEntry(i);
    // std::cout << electron_energy_radon << std::endl;
    h_radon->Fill(electron_energy_radon);
  }

  TFile *f_output= new TFile("spectra_1e.root","RECREATE");
